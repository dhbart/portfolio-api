...
class PromptBuilderTest {
    ...
    void preservesChunkOrderAndQuestion() {
        ...
        String prompt = builder.build("System", chunks, "What?", 100);
        assertThat(prompt).contains("System", "Context", "first\n\nsecond", "Question", "What?");
    }

    @Test
    void stopsBeforeContextExceedsConfiguredLength() {
        String prompt = builder.build("System", List.of(chunk("12345"), chunk("67890")), "Q", 7);
        assertThat(prompt).contains("12345").doesNotContain("67890");
    }

    @Test
    void includesInternalWarningWhenNoChunkExists() {
        assertThat(builder.build("System", List.of(), "Q", 10))
